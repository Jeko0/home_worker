...
function TasksCreator() {
  ...
  const [writersInfos, setWritersInfos] = useState([
    { user: { username: "choose category first" } },
  ]);
  ...
  useEffect(() => {
    ...
    if (currentCategories.length === 0 && subjects.length !== 0) {
      ...
      const result = categories.filter(
        (category) => category.subject_id === parseInt(subject_id)
      );
      if (result.length === 0) {
  ...
  return (
    <div className="w-full">
      ...
      <div className="flex flex-col justify-center items-center w-full">
        <form
          className="grid grid-flow-row w-3/4 mr-none"
        ...
        >
          <div className="flex flex-wrap mb-6 space-x-3">
            <div className="relative min-w-[30%]">
              <label>subject</label>
              <select
                onChange={handleSetCurrentSubject}
            ...
            <div className="relative min-w-[30%]">
              ...
              >
                {writersInfos.map((writersInfo) => (
                  <option onClick={() => setCurrentWritersInfo(writersInfo)}>
        ...
        <div className="flex items-stretch w-full h-full justify-center">
          {errors ? (
            <div className=" w-1/3 ">
              {errors.map((error) => (
                <div
                  className="alert-toast flex float-left bottom-0 mt-8 w-5/6 md:w-full max-w-sm"
                  onClick={() => handleCloseAlert(error)}
                >
                  <label
                    className="close cursor-pointer flex items-start justify-between w-full p-2 bg-red-600 h-24 rounded shadow-lg text-black"
                    title="close"
                  >
                    {error}
                    <svg
